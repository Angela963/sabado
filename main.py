...
language = "python"
version = input(
  "¿Qué versión mínima de Python quieres usar? (Ejemplo: 3.8): "
  ) or "3.8"

# TEST AUTOMATIZADOS
usar_test = input(
  "¿Quieres incluir tests automáticos? (S/n): "
  ) or "s"
steps_test = ''
if usar_test.lower() == 's':
    cmd_test = input(
      "¿Comando para ejecutar tus tests? (por defecto: 'pytest'): "
      ) or "pytest"
    steps_test = f"""
    - name: Ejecutar tests
      run: {cmd_test}"""
...
# ANALISIS ESTATICO
usar_linter = input(
  "¿Quieres análisis de código estático? (S/n): "
  ) or "s"
steps_linter = ''
linter_pkg = ''
if usar_linter.lower() == 's':
    ...
    cmd_linter = input(
      f"Comando para análisis estático (por defecto: '{linter} .'): "
      ) or f"{linter} ."
    steps_linter = f"""
...
usar_security = input(
  "¿Quieres análisis de seguridad? (S/n): "
  ) or "s"
steps_security = ''
security_pkg = ''
if usar_security.lower() == 's':
    security = input(
      "¿Herramienta para análisis de seguridad"
      ) or "bandit"
    security_pkg = security.split()[0]
    cmd_security = input(
      f"Comando para análisis de seguridad (por defecto: '{security} .'): "
      ) or f"{security} ."
    steps_security = f"""
